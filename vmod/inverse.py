...
class Inverse:
   ...
   def mcmc_em(self,name=None,move=None, nwalkers_per_dim = 2, use_uncertainty_hyperparam=True):
       ...
       Parameters:
           ...
           use_uncertainty_hyperparam: True to use an extra hyperparameter to auto-scale
                uncertainties.
            nwalkers_per_dim: use to control number of chains/walkers
       ...
       pos = np.array(inis) + 1e-4 * np.random.randn(nwalkers_per_dim*len(inis), len(inis)) #nwalkers per dim is a default of 2 to preserve orginal structure but i can edit it from my script now
      # pos = np.array(inis) + 1e-4 * np.random.randn(nwalkers_per_dim*1, 1) #nwalkers per dim is a default of 2 to preserve orginal structure but i can edit it from my script now

       #print(pos)
       print('Number of parameters: ', len(inis))
       ...
       if move is None:
           ...
           sampler = emcee.EnsembleSampler(nwalkers, ndim, self.log_probability, moves=moves, backend=backend)
       try:
